...
#http://www.cgafaq.info/wiki/Plucker_Coordinates also on the wikipedia of course
...
def triangle_ray_intersection(vertexA,vertexB,vertexC,rayPos,rayD):
  ...
  X=(1.0/(n.inner(P,e1)))*n.array([n.inner(Q,e2),n.inner(P,T),n.inner(Q,rayD)])
  return X	#t,u,v

def trianglePlueckerTest(vertexA,vertexB,vertexC,rayPos,rayD,epsilon=1e-10):
  e0=n.array((vertexA-vertexB).tolist()+(n.cross(vertexA,vertexB)).tolist())
  e1=n.array((vertexC-vertexA).tolist()+(n.cross(vertexC,vertexA)).tolist())
  e2=n.array((vertexB-vertexC).tolist()+(n.cross(vertexB,vertexC)).tolist())
  ...
  #if the signs are all the same it means the ray passes through the triangle
  #if any t0 is 0 or near 0 it means the ray passes throught the line or is parallel to the line (passes
  #through the line in infinity)
  if ((t0>0 and t1>0 and t2>0) or (t0<0 and t1<0 and t2<0)):
    print "Inside"
    return 1
  elif ((t0>-epsilon and t0<epsilon) or (t1>-epsilon and t1<epsilon) or (t2>-epsilon and t2<epsilon)):
    print "On line"
    return 0
  else:
    print "Outside"
...
def polygon_pluecker_test(vertex_list,ray_pos,ray_dir,epsilon=1e-10):
  ...
  for i,vertexi in enumerate(vertex_list):
    ...
    pluecker_list.append(n.concatenate((vertexj-vertexi,n.cross(vertexj,vertexi))))
