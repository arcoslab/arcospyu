...
def narray_to_kdltwist(narray):
    twist=PyKDL.Twist(PyKDL.Vector(narray[0],narray[1],narray[2]),PyKDL.Vector(narray[3],narray[4],narray[5]))
    return(twist)
...
def my_adddelta(frame,twist,dt):
    return(kdlframe_to_narray(PyKDL.addDelta(narray_to_kdlframe(frame),
                                       narray_to_kdltwist(twist),dt)))

def my_diff(frame1,frame2,dt):
    return(kdltwist_to_narray(PyKDL.diff(narray_to_kdlframe(frame1),
                                   narray_to_kdlframe(frame2),dt)))
...
def rpy_to_rot_matrix(roll_pitch_yaw):
    kdl_matrix=PyKDL.Rotation.RPY(roll_pitch_yaw[0], roll_pitch_yaw[1], roll_pitch_yaw[2])
