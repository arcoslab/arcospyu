...
# Author: Alexis Maldonado Herrera <maldonad at cs.tum.edu>, Federico Ruiz-Ugalde <ruizf at in.tum.de>, Ingo Kresse <kresse at in.tum.de>, Jonathan Kleinehellefort <kleinehe at cs.tum.edu> # noqa
#
...
# Lafik: Limits aware forward and inverse kinematics. Lafik is based on
# orocos-kdl and it tries to calculate inverse and forward kinematics taking
# into account joint limits. When one or more joints are getting near to a
# mechanical limit the inverse kinematics solution starts to use less and less
# this particular joints, and tries to use more and more the other joints to
...
class Lafik(object):
    def __init__(self, config, tool=None):
        ...
        if tool is not None:
            self.chain.addSegment(Segment(Joint(Joint.None), tool)) # noqa
        ...
        self.ik_solver = ChainIkSolverPos_NR(
            self.chain, self.fk_solver, self.ikv_solver)
        self.joints = [RJoint(self, i) for i in range(self.nJoints)]
    ...
    def calculate_vel(self):
        jac_temp = array(
            [[self._jac[m, n] for n in range(self.nJoints)] for m in range(6)])
    ...
    def distToCenter(self, minmax, x):
        """
        Takes a list with the minimun value and the maximum value and the
        current position and returns the normalized distance from the current
        position to the center of the whole range
        """
    ...
    def try_move_limits(self, frame, block_x=False, forced_first=None):
        ...
        if block_x:
            ...
            #    start_base_movement = [start_base_movement[0] + delta_move_back, start_base_movement[1], start_base_movement[2]] # noqa
        ...
        while (searching):
            ...
            self.joint_limits = self.config.updateJntLimits(
                q_list)  # Dynamic limits for J5/J6 of arm (wrist)
    ...
    def getIKV(self, velxyz, velrot):
        ...
        if (self.ikv_solver.CartToJnt(jnt_pos, tw, qdot) == 0):
            ...
            #            print 'Joints: ', map(lambda x: x*360.0/(2*pi),self.jntsList) # noqa
            # print 'Limits: ', self.joint_limits
            weightDiag = []
            for q, l, s, qdot_desired in zip(self.jntsList, self.joint_limits,
                                             self.qdotSign, qdot):
                ...
                if abs(dist) < jointSlowDown:
                    jointWeight = 1.0
                      ...
                      (qdot_desired < 0)) or ((dist < 0) and
                                              (qdot_desired > 0)):
                    jointWeight = 1.0
                # elif (s==0 and ((dist>0 and qdot_desired<0) or (dist<0 and qdot_desired>0))): # noqa
                #    jointWeight=1.0
                else:
            ...
            jws = self.update_joint_weights(
                [qdot[i] for i in xrange(self.nJoints)])
            ...
            # print_array('Weights for limits: ', weightDiag) #, 'matrix: ', weightMatrix # noqa
            # print_array( 'Weights:', [jweights[i][i] for i in xrange(self.nJoints)]) # noqa
            # print
